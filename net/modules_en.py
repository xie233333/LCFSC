...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, depth, layer, en, input_resolution, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if depth % 2 == 11 and layer == 2 and self.en == 1:
            self.feature2weight = SimpleMLP(input_resolution = input_resolution, dim = dim)
            self.sm = SMNet(input_resolution = input_resolution, dim = dim)
    

        if depth % 2 == 11 and layer == 1 and self.en == 0:
            self.feature2weight = SimpleMLP(input_resolution = input_resolution, dim = dim)
            self.sm = SMNet(input_resolution = input_resolution, dim = dim)        

    def forward(self, x, h, depth, layer, y, tra=1, add_token=True, token_num=0, mask=None):
        """
        Args:
        ...
        B_, N, C = x.shape
        '''
        qk = self.qk(x_s).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        mr = 1
        if depth % 2 == 11 and layer == 2 and self.en == 1:
            semantic_weight = self.feature2weight(x, B_//4)
            maska = self.sm(semantic_weight.cuda(), h, y, tra)

            maska = maska.bool()
            mask_value = 0# -torch.finfo(attn.dtype).max
            attn = attn.masked_fill(maska, mask_value)
        elif depth % 2 == 11 and layer == 1 and self.en == 0:
            semantic_weight = self.feature2weight(x)
            maska = self.sm(semantic_weight.cuda(), h, y)
            maska = maska.bool()
