...
from net.recurrent_cvae import CVAE
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, depth, layer, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            layer = layer, en = 1, input_resolution = self.input_resolution, qkv_bias=qkv_bias, qk_scale=qk_scale)
    ...
    def forward(self, x, x_s, h, depth, layer, y, tra):
        H, W = self.input_resolution
        B, L, C = x.shape
        ...
        attn_windows = self.attn(x_windows, h, depth, layer, y, tra, add_token=False, mask=self.attn_mask)            
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, x_s, h, layer, y, tra):
        if self.downsample is not None:
            x = self.downsample(x)
        for d in range(self.depth):
            x, x_s = self.blocks[d](x, x_s, h, d, layer, y, tra)
...
class LCFSC_Encoder(nn.Module):
    def __init__(self, img_size, patch_size, in_chans,
        ...
        self.hidden_dim = int(self.embed_dims[len(embed_dims)-1] * 1)
        ...
        self.head_list = nn.Sequential(
            nn.Linear(embed_dims[-1], C)
        )
        self.apply(self._init_weights)

    def forward(self, x, snr, model, h, y, tra):
        ...
        for i_layer, layer in enumerate(self.layers):
            x, x_s = layer(x, x_s.cuda(), h, i_layer, y, tra)
